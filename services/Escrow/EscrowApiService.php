...
class EscrowApiService
{
    ...
    /**
     * Return the authoritative escrow row for internal API operations.
     */
    public function getEscrowByReference(string $reference): ?array
    {
        ...
        try {
            ...
            Logger::write('escrow_api_service', [
                'step' => 'GET_ESCROW_BY_REFERENCE',
                'reference' => $reference,
                'found' => is_array($escrow),
            ]);

            return is_array($escrow) ? $escrow : null;
        } catch (Throwable $e) {
            Logger::write('escrow_api_service_error', [
                'step' => 'GET_ESCROW_BY_REFERENCE_EXCEPTION',
                'reference' => $reference,
                'message' => $e->getMessage(),
                'file' => $e->getFile(),
                'line' => $e->getLine(),
